...
\newpage
	...
	If there are nine shirts from county-level events, there are $34-9=25$ shirts that are \textbf{not} from a county-level event.
	
	\vspace{3mm}
	
	Number of ways to choose a four shirts that are \textbf{not} from a county-level event: 
	$$_{25}C_4=\frac{25!}{4!21!}=\frac{25\cdot 24\cdot 23\cdot 22\cdot \cancel{21!}}{4\cdot 3\cdot 2\cdot 1\cdot \cancel{21!}}=12,650\text{ ways}$$
		
	\vspace{3mm}
	
	$P(\text{chooses 4 not-county shirts})=\displaystyle\frac{_{25}C_4}{_{34}C_4}=\frac{12,650}{46,376}=0.2728$
	
	\vspace{3mm}
	\end{solution}
	
	\part What is the probability that he packs \textbf{at least} one shirt that is \textbf{not} from a county-level event?
	
	\begin{solution}[\stretch{1}]
	\begin{align*}
	P(\text{at least one non-county shirt}) &= 1-P(\text{all four county shirts})\\
	&= 1-\displaystyle\frac{_9C_4}{_{34}C_4}= 1-\frac{126}{46,376} \\
	&= 1-0.0027 = 0.9973\\
	\end{align*}
	\end{solution}
